...
@endphp
<div class="list-game border-b border-slate-800 relative">
    <div class="flex gap-3 flex-1 min-w-0 items-center">
        ...
        @else
            <div class="w-[60px] h-[80px] bg-slate-700 rounded flex-shrink-0 flex items-center justify-center text-slate-500 text-[10px]">No cover</div>
        @endif
        <div class="flex-1 min-w-0">
            {{-- Title + Scores --}}
            <div class="flex items-start gap-2 mb-1">
                <span class="font-bold text-primary-400 text-sm leading-tight truncate flex-1"><a href="/game/{{ $game->slug }}" class="after:absolute after:inset-0">{{ $game->title }}</a></span>
            ...
            <div class="flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[11px] mb-3">
                ...
                @endif
                @if($game->time_min)
                    <span><span class="font-bold text-gray-300">{{ $game->time_range }}</span></span>
                @endif
            ...
            {{-- Guide links --}}
            @if(!empty($guides))
                <div class="relative z-10 flex items-center gap-1 px-1.5 py-0.5 bg-slate-700/50 rounded-lg border border-slate-600 w-fit">
    ...
    @if($game->description)
        <div class="hidden lg:flex items-center w-64 xl:w-80 shrink-0 pl-4 border-l border-slate-700/50">
            <p class="text-xs text-gray-500 line-clamp-3 leading-relaxed">{{ $game->description }}</p>
        </div>
    @endif
</div>
